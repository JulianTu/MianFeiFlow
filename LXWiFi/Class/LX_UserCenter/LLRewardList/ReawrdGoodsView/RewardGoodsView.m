...
#define kRVviewW    (kmainScreenWidth -3*kOffX_2_0)/2
#define kRVimgHeight     87.0f
#define kRVviewH         146.0f
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        _leftView = [[UIView alloc] initWithFrame:CGRectMake(kOffX_2_0, 0, kRVviewW, kRVviewH)];
        _leftView.clipsToBounds = YES;
        _leftView.hidden = NO;

        _rightView = [[UIView alloc] initWithFrame:CGRectMake( 2*kOffX_2_0 +kRVviewW, 0, kRVviewW, kRVviewH)];
        _rightView.clipsToBounds = YES;
        _rightView.hidden = YES;
        
        //加载左边产品图片
        _leftImage = [[UIImageView alloc]initWithFrame:CGRectMake(0, kOffX_2_0, kRVviewW, kRVimgHeight)];
        _leftImage.clipsToBounds = YES;
        _leftImage.userInteractionEnabled =NO;
        [_leftView addSubview:_leftImage];
        //加载右边产品图片
        _rightImage = [[UIImageView alloc]initWithFrame:CGRectMake(0, kOffX_2_0, kRVviewW, kRVimgHeight)];
        _rightImage.clipsToBounds = YES;
        _rightImage.userInteractionEnabled =NO;
        [_rightView addSubview:_rightImage];
        
        //加载左边产品名称
        _leftNameLab = [self loadWithNameLab:CGRectMake(0, _leftImage.frame.size.height+kOffX_2_0+_leftImage.frame.origin.y, kRVviewW, 13.0)];
        [_leftView addSubview:_leftNameLab];
        //加载右边产品名称
        _rightNameLab = [self loadWithNameLab:CGRectMake(0, _rightImage.frame.size.height+_rightImage.frame.origin.y +kOffX_2_0, kRVviewW, 13.0)];
...
-(void)initCellContent{
    ...
    _leftNeedBeanLab.text = [NSString stringWithFormat:@"%d流量币",self.leftGoods.needBean];
    NSString *needBeanStr =[NSString stringWithFormat:@"%d",self.leftGoods.needBean];
    NSMutableAttributedString *attString =[[NSMutableAttributedString alloc] initWithString:_leftNeedBeanLab.text];
    [attString addAttribute:NSFontAttributeName value:[UIFont fontWithName:@"Helvetica-Bold" size:16.0] range:NSMakeRange(0, needBeanStr.length)];
    [attString addAttribute:NSFontAttributeName value:kFontSize_11 range:NSMakeRange(needBeanStr.length,3)];
    [attString addAttribute:NSForegroundColorAttributeName value:KOrangeColor2_0 range:NSMakeRange(0, needBeanStr.length +3)];
    _leftNeedBeanLab.attributedText = attString;
    [_leftNeedBeanLab sizeToFit];
    
    _rightNeedBeanLab.text = [NSString stringWithFormat:@"%d流量币",self.rightGoods.needBean];
    NSString *needBeanStr2 =[NSString stringWithFormat:@"%d",self.rightGoods.needBean];
    NSMutableAttributedString *attString2 =[[NSMutableAttributedString alloc] initWithString:_rightNeedBeanLab.text];
    [attString2 addAttribute:NSFontAttributeName value:[UIFont fontWithName:@"Helvetica-Bold" size:16.0] range:NSMakeRange(0, needBeanStr2.length)];
    [attString2 addAttribute:NSFontAttributeName value:kFontSize_11 range:NSMakeRange(needBeanStr2.length,3)];
    [attString2 addAttribute:NSForegroundColorAttributeName value:KOrangeColor2_0 range:NSMakeRange(0, needBeanStr2.length +3)];
    _rightNeedBeanLab.attributedText = attString2;
