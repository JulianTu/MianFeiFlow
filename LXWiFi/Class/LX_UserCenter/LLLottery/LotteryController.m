...
-(void) initLotteryContentView{
    ...
    float offx = 8.0f ;
    UIImageView *titleImg = [self loadImageViewFrame:CGRectMake( kmainScreenWidth/2 -title.size.width/2, 2.0f, title.size.width, title.size.height) andImage:title];
    ...
    textOne = [[TaoJinLabel alloc] initWithFrame:CGRectMake( labOne.frame.origin.x +labOne.frame.size.width -6.0f, labOne.frame.origin.y +1.0f, titleImg.frame.size.width /2-offx -labOne.frame.size.width, 15.0f) text:[NSString stringWithFormat:@"0"] font:GetBoldFont(14.0f) textColor:kWitheColor textAlignment:NSTextAlignmentCenter numberLines:1];
//    textOne.backgroundColor =[UIColor redColor];
    
    TaoJinLabel *labTwo = [[TaoJinLabel alloc] initWithFrame:CGRectMake( titleImg.frame.origin.x +titleImg.frame.size.width/2 -5.0f, titleImg.frame.size.height/2 -10.0f +titleImg.frame.origin.y, titleImg.frame.size.width/2, 14.0f) text:@"完成任务" font:GetFont(14.0f) textColor:cofferColor textAlignment:NSTextAlignmentLeft numberLines:1];
    [labTwo sizeToFit];
    labTwo.frame =CGRectMake(labTwo.frame.origin.x, labTwo.frame.origin.y, labTwo.frame.size.width, labTwo.frame.size.height);
    
    textTwo = [[TaoJinLabel alloc] initWithFrame:CGRectMake(labTwo.frame.origin.x +labTwo.frame.size.width +18.0f, labTwo.frame.origin.y +1.0f , titleImg.frame.size.width/2 -offx-labTwo.frame.size.width, 15.0f) text:[NSString stringWithFormat:@"13"] font:GetBoldFont(14.0f) textColor:kWitheColor textAlignment:NSTextAlignmentCenter numberLines:1];
//    textTwo.backgroundColor =[UIColor redColor];
    UIImage *lottery =GetImage(@"lotteryBG");
    lotteryImg = [self loadImageViewFrame:CGRectMake(kmainScreenWidth/2 -lottery.size.width/2, titleImg.frame.origin.y +titleImg.frame.size.height  +10.0f, lottery.size.width, lottery.size.height) andImage:lottery];
    ...
    UIImageView *markImg = [self loadImageViewFrame:CGRectMake(kmainScreenWidth/2 -mark.size.width/2, lotteryImg.frame.origin.y +40.0f, mark.size.width, mark.size.height) andImage:mark];
...
//    float panSize = 231.0f;
    lotteryPan.frame =CGRectMake(kmainScreenWidth/2 -pan.size.width/2, markImg.frame.origin.y +22.0,pan.size.width, pan.size.height);
    ...
    rewardedTip =[[TaoJinLabel alloc] initWithFrame:CGRectMake(0, lotteryImg.frame.origin.y +lotteryImg.frame.size.height -30.0f, kmainScreenWidth, 16.0f) text:@"祝君好运，恭喜发财" font:GetFont(16.0) textColor:ColorRGB(15.0, 106.0, 179.0, 1.0) textAlignment:NSTextAlignmentCenter numberLines:1];
...
-(void)stopedAnimation{
    ...
    if (todayCount >0) {
        ...
        textTwo.text =[NSString stringWithFormat:@"%d",missionCount-1];
        missionCount -=1;
    }
    NSDictionary *dic =@{IncomeNoticKey: NSStringFromInt(rewardGold)};
...
//            [self requestForDailyLottery];
    ...
    }else if (alertView.tag == 30001){     //抽奖失败
        [alertView dismissWithClickedButtonIndex:0 animated:YES];
    }
