...
#define segItemW                                    76.0f
#define segItemH                                    30.0f
//#define kLineColor2_0                                   [UIColor clearColor]
...
- (id)initWithFrame:(CGRect)frame btnAry:(NSArray *)btnStrAry btnAction:(SegmentActionBlock)btnAction slidColor:(UIColor *)slidColor viewAry:(NSArray *)viewAry headView:(HeadToolBar *)headView{
 ...
 *
 */
- (id)initWithFrame:(CGRect)frame btnAry:(NSArray *)btnStrAry btnAction:(SegmentActionBlock)segAction slidColor:(UIColor *)slidColor viewAry:(NSArray *)viewAry{
...
//            [self addSubview:btn];
       ...
       */
        _segmented =[[UISegmentedControl alloc] initWithItems:btnStrAry];
        _segmented.frame = CGRectMake(kmainScreenWidth/2 - segItemW*btnStrAry.count/2, btnHeight/2- segItemH/2, segItemW *btnStrAry.count, segItemH);
        [_segmented addTarget:self action:@selector(segmentAction:) forControlEvents:UIControlEventValueChanged];
        _segmented.selectedSegmentIndex =0;
        _segmented.tintColor = kWitheColor;
        [self addSubview:_segmented];
        
        self.backgroundColor = slidColor;
        ...
        self.scrollView.backgroundColor = kWitheColor;
...
//        [self addSubview:self.slidView];
        ...
        //公用
        currentPage = 0;
        _segAction = [segAction copy];
    }
    return self;
}
-(void)segmentAction:(UISegmentedControl *)segment {
    
    [_scrollView setContentOffset:CGPointMake(segment.selectedSegmentIndex *kmainScreenWidth, 0) animated:YES];
   _segAction(segment);
}
/**
 *  返回上方横栏按钮的高度值
...
//    _btnAction(button);
    ...
    oldBtnTag = currentPage + 1;
     [_segmented setSelectedSegmentIndex:currentPage];
    _segAction(_segmented);
