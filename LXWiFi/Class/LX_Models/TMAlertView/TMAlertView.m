...
#define kLineOffY   40.0f
#define kCodeViewHeight  300.0f
#define kCodeImgSize  160.0f
#define kUserPicImgSize  35.0f

#define kViewOffx        25.0f
#define kViewWeith      kmainScreenWidth -2*kViewOffx
#define kViewHeigth     222.0f
#define kYOff           20.0f
#define kYOff2          40.0f
...
//        [self addSubview:self.dengpao];
        ...
        self.alertViewTitle =[[UILabel alloc]initWithFrame:CGRectMake(0, kYOff, kViewWeith, 16.0)];
        ...
        self.alertViewTitle.textColor =kBlackTextColor;
        [self addSubview:self.alertViewTitle];
        
        self.oneContent =[[UILabel alloc]initWithFrame:CGRectMake(0, self.alertViewTitle.frame.origin.y +self.alertViewTitle.frame.size.height +kYOff, kViewWeith, 11.0)];
        ...
        self.oneContent.textColor =kBlackTextColor;

        [self addSubview:self.oneContent];
        
        self.twoContent =[[UILabel alloc]initWithFrame:CGRectMake(0, self.oneContent.frame.origin.y+ kYOff, kViewWeith, 11.0)];
        self.twoContent.textAlignment =NSTextAlignmentCenter;
        ...
        self.twoContent.textColor =kBlackTextColor;

        [self addSubview:self.twoContent];
        
        self.threeContent =[[UILabel alloc]initWithFrame:CGRectMake(0, self.twoContent.frame.origin.y+ kYOff, kViewWeith, 11.0)];
        self.threeContent.textAlignment =NSTextAlignmentCenter;
        ...
        self.threeContent.textColor =kBlackTextColor;
        [self addSubview:self.threeContent];
        
        self.fourContent =[[UILabel alloc]initWithFrame:CGRectMake(0, self.threeContent.frame.origin.y+ kYOff, kViewWeith, 11.0)];
        self.fourContent.textAlignment =NSTextAlignmentCenter;
        ...
        self.fourContent.textColor =kBlackTextColor;
        [self addSubview:self.fourContent];
        
        self.tipContent =[[UILabel alloc]initWithFrame:CGRectMake(0, self.fourContent.frame.origin.y +self.fourContent.frame.size.height+ kYOff, kViewWeith, 11.0)];
        self.tipContent.textAlignment =NSTextAlignmentLeft;
        ...
        [self.tipContent sizeToFit];
        float viewW = kViewWeith ;
        self.tipContent.frame = CGRectMake(viewW/2 - self.tipContent.frame.size.width/2 +5.0f, self.tipContent.frame.origin.y, self.tipContent.frame.size.width, self.tipContent.frame.size.height);
        UITapGestureRecognizer *tap =[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(onClickedCheck)];
        [self.tipContent addGestureRecognizer:tap];
        self.tipContent.userInteractionEnabled =YES;
...
//        [self addSubview:self.tipButton];
        ...
        self.checkImage.frame =CGRectMake(self.tipContent.frame.origin.x -tipImage.size.width -7.0f, self.tipContent.frame.origin.y-(tipImage.size.height/2 -self.tipContent.frame.size.height/2), tipImage.size.width, tipImage.size.height);
        ...
        UIView* lineView =[self loadSeprateLineWithFrame:CGRectMake(0, self.checkImage.frame.origin.y +self.checkImage.frame.size.height +15.0f, kViewWeith, LineWidth)];
        [self addSubview:lineView];
        
        self.okContent =[UIButton buttonWithType:UIButtonTypeCustom];
        self.okContent.frame =CGRectMake(0, lineView.frame.origin.y +lineView.frame.size.height, kViewWeith, SendButtonHeight);
        ...
        self.okContent.titleLabel.font =GetBoldFont(16.0);
        UIColor *blue = ColorRGB(21.0, 126.0, 251.0, 1.0);
        self.okContent.titleLabel.textColor =blue;
        [self.okContent setBackgroundImage:[UIImage createImageWithColor:KLightGrayColor2_0] forState:UIControlStateHighlighted];
        [self.okContent setTitleColor:blue forState:UIControlStateNormal];
        [self.okContent setTitleColor:blue forState:UIControlStateHighlighted];
...
-(UIButton *)loadBtnWithFrame:(CGRect) frame andTitle:(NSString *)title{
    ...
    [btn setTitleColor:kBlueTextColor forState:UIControlStateHighlighted];
    return btn;
}
-(id)initWithTitle:(NSString *)title andUserPic:(NSData *)userPicData andProduceImg:(UIImage *)img andIntroduce:(NSString *)intro okBtnTitle:(NSString *)okTitle cancleTitle:(NSString *)cancelTitle{
    ...
    if (self) {
        ...
        if (![NSString isEmptyString:cancelTitle] && ![NSString isEmptyString:okTitle]) {
            for (int i=0; i<2; i ++) {
...
- (void)show
{
    ...
    self.frame = CGRectMake(kViewOffx, (CGRectGetHeight(topVC.view.bounds) - kViewHeigth) * 0.5, kViewWeith, kViewHeigth);
