...
- (void)setImageWithURL:(NSURL *)url refreshCache:(BOOL)refreshCache needSetViewContentMode:(BOOL)needMode needBgColor:(BOOL)needBgColor placeholderImage:(UIImage *)placeholder withImageSize:(CGSize)imageSize {
    ...
    UIImageView *defImage ;
    if (!CGSizeEqualToSize(CGSizeZero, imageSize)) {
        self.image =nil;
        if(imageSize.width > self.image.size.width || imageSize.height > self.image.size.height){
//            self.image = [self imageByScalingToSize:CGSizeMake(imageSize.width, imageSize.height) sourceImage:placeholder];
            defImage =[[UIImageView alloc] initWithImage:placeholder];
            defImage.frame = CGRectMake(self.frame.size.width/2 -placeholder.size.width/2, self.frame.size.height/2 -placeholder.size.height/2, placeholder.size.width, placeholder.size.height);
            [self addSubview:defImage];
//            [self setContentMode:UIViewContentModeScaleAspectFit];
    ...
    if (needBgColor) {
        [self setBackgroundColor:ColorRGB(200.0, 200.0, 200.0, 1.0)];
    }
    [self setClearsContextBeforeDrawing:YES];
    if (needMode) {
//        [self setContentStretch:CGRectMake(0.3, 0.3, 0.4, 0.4)];
       self.image= [self.image resizableImageWithCapInsets:UIEdgeInsetsMake(5.0, 5.0, 5.0, 5.0) resizingMode:UIImageResizingModeStretch];
